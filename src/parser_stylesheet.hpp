...
namespace Sass {
  ...
  public:
    ...
    AtRootRule* _atRootRule(Offset start); // CssAtRootRule

    // Consumes a query expression of the form `(foo: bar)`.
    Interpolation* _atRootQuery();

    ContentRule* _contentRule(Offset start); // ContentRule

    DebugRule* _debugRule(Offset start); // DebugRule

    uint8_t exclusive;

    bool _forRuleUntil();

    Each* _eachRule(Offset start, Statement* (StylesheetParser::* child)()); // EachRule
    
    ErrorRule* _errorRule(Offset start); // ErrorRule

    ExtendRule* _extendRule(Offset start);

    FunctionRule* _functionRule(Offset start); // FunctionRule

    For* _forRule(Offset start, Statement* (StylesheetParser::* child)()); // ForRule
    ...
    MediaRule* mediaRule(Offset start);

    MixinRule* _mixinRule2(Offset start); // MixinRule

    AtRule* mozDocumentRule(Offset start, Interpolation* name);

    Return* _returnRule(Offset start); // ReturnRule

    // SupportsRule
    SupportsRule* supportsRule(Offset start);

    // UseRule _useRule(StringScannerState start);

    WarnRule* _warnRule(Offset start); // WarnRule

    WhileRule* _whileRule(Offset start, Statement* (StylesheetParser::* child)()); // WhileRule

    AtRule* unknownAtRule(Offset start, Interpolation* name);

    Statement* _disallowedAtRule(Offset start);
    ...
    Expression* _map(Expression* first, Offset start); // MapExpression

    Expression* _hashExpression();

    Color* _hexColorContents(StringScannerState start); // ColorExpression
    ...
    Interpolation* _tryUrlContents(const Offset& start, sass::string name = "");
    ...
    void interpolatedIdentifierBody(InterpolationBuffer& buffer);
