...
namespace Sass {
  ...
  namespace Functions {
    ...
    BUILT_IN(set_nth)
    {
      ...
      List_Ptr result = SASS_MEMORY_NEW(List, pstate, l->length(), l->separator(), l->delimiter());
    ...
    BUILT_IN(join)
    {
      ...
      Value_Obj bracketed = ARG("$bracketed", Value);
      ...
      String_Constant_Ptr bracketed_as_str = SASS_MEMORY_CAST(String_Constant, bracketed);
      ...
      List_Obj result = SASS_MEMORY_NEW(List, pstate, len, sep_val, delimiter);
      result->concat(&l1);
      result->concat(&l2);
    ...
    BUILT_IN(is_bracketed)
    {
      ...
      bool bracketed = list && list->is_bracketed();
      return SASS_MEMORY_NEW(Boolean, pstate, bracketed);
    }
