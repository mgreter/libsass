...
namespace Sass {
  ...
  namespace Functions {
    ...
    template <>
    Selector_List_Ptr get_arg_sel(const std::string& argname, Env& env, Signature sig, ParserState pstate, Backtrace* backtrace, Context& ctx) {
    ...
    template <>
    Complex_Selector_Ptr get_arg_sel(const std::string& argname, Env& env, Signature sig, ParserState pstate, Backtrace* backtrace, Context& ctx) {
      Expression_Ptr exp = ARG(argname, Expression);
      if (exp->concrete_type() == Expression::NULL_VAL) {
        std::stringstream msg;
        msg << argname << ": null is not a valid selector: it must be a string,\n";
      ...
      std::string exp_src = exp->to_string(ctx.c_options) + "{";
      Selector_List_Ptr sel_list = Parser::parse_selector(exp_src.c_str(), ctx);
      return (sel_list->length() > 0) ? sel_list->first() : 0;
    }

    template <>
    Compound_Selector_Ptr get_arg_sel(const std::string& argname, Env& env, Signature sig, ParserState pstate, Backtrace* backtrace, Context& ctx) {
      Expression_Ptr exp = ARG(argname, Expression);
      if (exp->concrete_type() == Expression::NULL_VAL) {
        std::stringstream msg;
    ...
    BUILT_IN(length)
    {
      if (Selector_List_Ptr sl = dynamic_cast<Selector_List_Ptr>(env["$list"])) {
      ...
      if (v->concrete_type() == Expression::SELECTOR) {
        ...
        } else if (Selector_List_Ptr ls = dynamic_cast<Selector_List_Ptr>(v)) {
    ...
    BUILT_IN(selector_nest)
    {
      ...
      std::vector<Selector_List_Ptr> parsedSelectors;
      for (size_t i = 0, L = arglist->length(); i < L; ++i) {
        ...
        std::string exp_src = exp->to_string(ctx.c_options) + "{";
        Selector_List_Ptr sel = Parser::parse_selector(exp_src.c_str(), ctx);
      ...
      std::vector<Selector_List_Ptr>::iterator itr = parsedSelectors.begin();
      Selector_List_Ptr result = *itr;
      ++itr;

      for(;itr != parsedSelectors.end(); ++itr) {
        Selector_List_Ptr child = *itr;
    ...
    BUILT_IN(selector_append)
    {
      ...
      for(;itr != parsedSelectors.end(); ++itr) {
        Selector_List_Ptr child = *itr;
        std::vector<Complex_Selector_Ptr> newElements;
    ...
    {
      Selector_List_Ptr selector1 = ARGSEL("$selector1", Selector_List, p_contextualize);
      Selector_List_Ptr selector2 = ARGSEL("$selector2", Selector_List, p_contextualize);
    ...
    {
      Compound_Selector_Ptr sel = ARGSEL("$selector", Compound_Selector, p_contextualize);
    ...
    {
      Selector_List_Ptr selector = ARGSEL("$selector", Selector_List, p_contextualize);
      Selector_List_Ptr extendee = ARGSEL("$extendee", Selector_List, p_contextualize);
      Selector_List_Ptr extender = ARGSEL("$extender", Selector_List, p_contextualize);
    ...
    {
      Selector_List_Ptr selector = ARGSEL("$selector", Selector_List, p_contextualize);
      Selector_List_Ptr original = ARGSEL("$original", Selector_List, p_contextualize);
      Selector_List_Ptr replacement = ARGSEL("$replacement", Selector_List, p_contextualize);

      ExtensionSubsetMap subset_map;
      replacement->populate_extends(original, ctx, subset_map);

      Selector_List_Ptr result = Extend::extendSelectorList(selector, ctx, subset_map, true);
    ...
    {
      Selector_List_Ptr sel = ARGSEL("$selector", Selector_List, p_contextualize);
    ...
    {
      Selector_List_Ptr sel_sup = ARGSEL("$super", Selector_List, p_contextualize);
      Selector_List_Ptr sel_sub = ARGSEL("$sub", Selector_List, p_contextualize);
