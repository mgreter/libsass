...
namespace Sass {
  ...
  namespace Functions {
    ...
    BUILT_IN(rgba_2)
    {
      ...
      Color_Ptr new_c = SASS_MEMORY_NEW(Color, *c_arg);
    ...
    BUILT_IN(to_upper_case)
    {
      ...
      if (String_Quoted_Ptr ss = SASS_MEMORY_CAST_PTR(String_Quoted, s)) {
        String_Quoted_Ptr cpy = SASS_MEMORY_NEW(String_Quoted, *ss);
        cpy->value(str);
        return cpy;
      } else {
        return SASS_MEMORY_NEW(String_Quoted, pstate, str);
    ...
    {
      Number_Ptr n = ARG("$number", Number);
      Number_Ptr r = SASS_MEMORY_NEW(Number, *n);
      r->pstate(pstate);
...
// return l.survive();
    ...
    BUILT_IN(zip)
    {
      List_Obj arglist = SASS_MEMORY_NEW(List, *ARG("$lists", List));
      size_t shortest = 0;
      for (size_t i = 0, L = arglist->length(); i < L; ++i) {
    ...
    BUILT_IN(map_get)
    {
      ...
      try {
        ...
        return val ? val.survive() : SASS_MEMORY_NEW(Null, pstate);
    ...
    BUILT_IN(keywords)
    {
      List_Obj arglist = SASS_MEMORY_NEW(List, *ARG("$args", List));
    ...
    BUILT_IN(call)
    {
      ...
      for (size_t i = 0, L = arglist->length(); i < L; ++i) {
        ...
        if (arglist->is_arglist()) {
          ...
          *args << SASS_MEMORY_NEW(Argument, pstate, expr);
        }
      }
      ...
      for(;itr != parsedSelectors.end(); ++itr) {
        ...
        for (size_t i = 0, resultLen = result->length(); i < resultLen; ++i) {
          for (size_t j = 0, childLen = child->length(); j < childLen; ++j) {
            Complex_Selector_Obj parentSeqClone = (*result)[i]->clone2();
    ...
    BUILT_IN(simple_selectors)
    {
      ...
      for (size_t i = 0, L = sel->length(); i < L; ++i) {
        ...
        *l << SASS_MEMORY_NEW(String_Quoted, ss->pstate(), ss_string);
      }
