...
on SassException catch (error) {
      ...
      */
      ...
      for (ComplexSelector_Obj complex : selectors) {
        ...
        Extension withExtender2 = extension.withExtender(complex);
        ...
        if (sources.find(complex) != sources.end()) {
          ...
          Extension merged = mergeExtension(existingExtension->second, withExtender2);
  ...
  Extension Extender::extensionForSimple(Simple_Selector_Obj simple)
  {
  ...
  Extension Extender::extensionForCompound(std::vector<Simple_Selector_Obj> simples)
  {
...
/////    // std::cerr << "CHECK HERE " << targetsUsed.size() << " vs " << extensions.size() << "\n";
    ...
    std::vector<std::vector<Extension>> prePaths = paths(options);
  ...
  std::vector<Extension> Extender::extendWithoutPseudo(Simple_Selector_Obj simple, ExtSelExtMap& extensions, ExtSmplSelSet* targetsUsed) {
  ...
  std::vector<std::vector<Extension>> Extender::extendSimple(Simple_Selector_Obj simple, ExtSelExtMap& extensions, Media_Block_Obj mediaQueryContext, ExtSmplSelSet* targetsUsed) {
    ...
    std::vector<Extension> result = extendWithoutPseudo(simple, extensions, targetsUsed);
