...
on SassException catch (error) {
  ...
  redo:
    ...
    if (set.find(complex1) == set.end()) {
      ...
      ComplexSelector_Obj asd = set.front();
      ComplexSelector_Obj qwe = set.back();
      // debug_ast(asd);
      // debug_ast(qwe);
      // debug_ast(complex1);
      // std::cerr << "FRONT " << debug_vec(asd) << "\n";
      // std::cerr << "BACK " << debug_vec(qwe) << "\n";
      // std::cerr << "KEY " << debug_vec(complex1) << "\n";

      // std::cerr << "front == " << (*complex1 == *asd ? "true" : "false") << "\n";
      // std::cerr << "back == " << (*complex1 == *qwe ? "true" : "false") << "\n";
