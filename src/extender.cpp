...
namespace Sass {
  ...
  void Extender::addSelector(SelectorList_Obj selector)
  {
    ...
    for (auto complex : selector->elements()) {
      ...
      std::cerr << "SELECTOR IS INVISIBLE\n";
    }
  ...
  std::vector<ComplexSelector_Obj> Extender::extendComplex(ComplexSelector_Obj complex, ExtSelExtMap& extensions)
  {
    ...
    for (std::vector<ComplexSelector_Obj> path : sels) {
      ...
      for (std::vector<CompoundOrCombinator_Obj> components : ww) {
        ...
        if (first) std::cerr << "HAS FIRST " << debug_vec(complex) << "\n";
        if (first) std::cerr << "BEFORE " << debug_vec(originals) << "\n";
...
/////    std::cerr << "CHECK HERE " << targetsUsed.size() << " vs " << extensions.size() << "\n";
  ...
  std::vector<std::vector<Extension2>> Extender::extendSimple(Simple_Selector_Obj simple, ExtSelExtMap& extensions, ExtSmplSelSet* targetsUsed) {
    ...
    if (Pseudo_Selector_Obj pseudo = Cast<Pseudo_Selector>(simple)) {
      if (pseudo->selector2()) {
        ...
        std::vector<std::vector<Extension2>> ret;
        for (auto pseudo : extended) {
          std::cerr << "CALL withoutPseudo " << debug_vec(pseudo) << "\n";
          auto vec = extendWithoutPseudo(pseudo, extensions, targetsUsed);

          if (!vec.empty()) {
            std::cerr << "withoutPseudo RV " << debug_vec(vec) << "\n";
            ret.insert(ret.end(), vec);
          }
          else {
            std::vector<Extension2> ext = { extensionForSimple(pseudo) };
            std::cerr << "extensionForSimple " << debug_vec(ext) << "\n";
            ret.insert(ret.end(), { ext });
          }
        }
        std::cerr << "EARLY RET " << debug_vec(ret) << "\n";
        return ret;
  ...
  std::vector<ComplexSelector_Obj> extendPseudoComplex(ComplexSelector_Obj complex, Pseudo_Selector_Obj pseudo) {
    ...
    std::cerr << "extendPseudoComplex2 " << debug_vec(complex) << "\n";
  ...
  std::vector<Pseudo_Selector_Obj> Extender::extendPseudo(Pseudo_Selector_Obj pseudo, ExtSelExtMap& extensions)
  {
    ...
    std::cerr << "EXPANDING COMPLEXES " << debug_vec(complexes) << "\n";
    ...
    if (pseudo->normalized() == "not" && pseudo->selector2()->length() == 1) {
      ...
      auto ret = pseudo->withSelector(list->concat(complexes));
      std::cerr << "RETURN EXPANDED " << debug_vec(ret) << "\n";
      return { ret };
    }
