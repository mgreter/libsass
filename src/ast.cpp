...
//    Compound_Selector_Obj unleak = 0;
  ...
  bool Selector_List_Ref::operator== (const Selector& rhs) const
  {
  ...
  bool Selector_List_Ref::operator==(const Expression& rhs) const
  {
    // solve the double dispatch problem by using RTTI information via dynamic cast
  ...
  bool Selector_List_Ref::operator== (const Selector_List& rhs) const
  {
    // for array access
    size_t i = 0, n = 0;
    size_t iL = length();
  ...
  Compound_Selector_Ptr Simple_Selector::unify_with(Compound_Selector_Ptr rhs, Context& ctx)
  {
    ...
    if (!found)
    {
      rhs->append(this);
      return rhs;
      Compound_Selector_Ptr cpy = SASS_MEMORY_NEW(Compound_Selector, *rhs);
      cpy->append(this);
      return cpy;
    }
...
return rhs;
    ...
    Compound_Selector_Ptr cpy = SASS_MEMORY_NEW(Compound_Selector, rhs->pstate());
  ...
  Compound_Selector_Ptr Element_Selector::unify_with(Compound_Selector_Ptr rhs, Context& ctx)
  {
    ...
    if (typeid(*rhs_0) == typeid(Element_Selector))
    {
      ...
      // Compound_Selector_Ptr copy = SASS_MEMORY_NEW(Compound_Selector, *rhs);
  ...
  Compound_Selector_Ptr Id_Selector_Ref::unify_with(Compound_Selector_Ptr rhs, Context& ctx)
  {
    for (size_t i = 0, L = rhs->length(); i < L; ++i)
    {
  ...
  Compound_Selector_Ptr Pseudo_Selector_Ref::unify_with(Compound_Selector_Ptr rhs, Context& ctx)
  {
  ...
  bool Attribute_Selector_Ref::operator< (const Attribute_Selector& rhs) const
  {
    ...
    if (is_ns_eq(ns(), rhs.ns())) {
      return (name() < rhs.name()) || (matcher() < rhs.matcher())
  ...
  bool Compound_Selector_Ref::is_superselector_of(Selector_List_Obj rhs, std::string wrapped)
  {
  ...
  Complex_Selector_Obj Compound_Selector_Ref::to_complex()
  {
  ...
  Selector_List_Ptr Complex_Selector_Ref::unify_with(Complex_Selector_Ptr other, Context& ctx)
  {
      ...
      if (last()->combinator() != ANCESTOR_OF && c != ANCESTOR_OF) {
        ...
        if (SASS_MEMORY_CAST(Element_Selector, h->first())) {
          if (Class_Selector_Ptr sq = SASS_MEMORY_CAST(Class_Selector, rh->last())) {
            Class_Selector_Ptr sqs = SASS_MEMORY_NEW(Class_Selector, *sq);
            ...
            Id_Selector_Ptr sqs = SASS_MEMORY_NEW(Id_Selector, *sq);
            sqs->name(sqs->name() + (*h)[0]->name());
            sqs->pstate((*h)[0]->pstate());
            (*rh)[rh->length()-1] = sqs;
            rh->pstate(h->pstate());
            ...
            Element_Selector_Ptr tss = SASS_MEMORY_NEW(Element_Selector, *ts);
            ...
            Placeholder_Selector_Ptr pss = SASS_MEMORY_NEW(Placeholder_Selector, *ps);
  ...
  Selector_List_Ptr Selector_List_Ref::resolve_parent_refs(Context& ctx, Selector_List_Ptr ps, bool implicit_parent)
  {
  ...
  Selector_List_Ptr Complex_Selector_Ref::resolve_parent_refs(Context& ctx, Selector_List_Ptr parents, bool implicit_parent)
  {
  ...
  Selector_List_Ptr Complex_Selector_Ref::tails(Context& ctx, Selector_List_Ptr tails)
  {
  ...
  void Complex_Selector_Ref::cloneChildren()
  {
    if (head()) head(head()->clone2());
    if (tail()) tail(tail()->clone2());
  }

  void Compound_Selector_Ref::cloneChildren()
  {
    for (size_t i = 0, l = length(); i < l; i++) {
      at(i) = at(i)->clone2();
    }
  ...
  void Selector_List_Ref::cloneChildren()
  {
    for (size_t i = 0, l = length(); i < l; i++) {
      at(i) = at(i)->clone2();
    }
  ...
  {
    selector(selector()->clone2());
  }

  // remove parent selector references
  // basically unwraps parsed selectors
  void Selector_List_Ref::remove_parent_selectors()
  {
  ...
  bool Selector_List_Ref::has_parent_ref()
  {
  ...
  bool Selector_List_Ref::has_real_parent_ref()
  {
    for (Complex_Selector_Obj s : elements()) {
      if (s && s->has_real_parent_ref()) return true;
    }
  ...
  void Selector_List_Ref::adjust_after_pushing(Complex_Selector_Obj c)
  {
  ...
  bool Complex_Selector_Ref::is_superselector_of(Selector_List_Obj sub, std::string wrapping)
  {
    // Check every rhs selector against left hand list
  ...
  // it's a superselector if every selector of the right side
  // list is a superselector of the given left side selector
  bool Selector_List_Ref::is_superselector_of(Selector_List_Obj sub, std::string wrapping)
  {
    // Check every rhs selector against left hand list
    for(size_t i = 0, L = sub->length(); i < L; ++i) {
      if (!is_superselector_of(&(*sub)[i], wrapping)) return false;
  ...
  bool Selector_List_Ref::is_superselector_of(Compound_Selector_Obj sub, std::string wrapping)
  {
  ...
  // it's a superselector if every selector on the right side
  // is a superselector of any one of the left side selectors
  bool Selector_List_Ref::is_superselector_of(Complex_Selector_Obj sub, std::string wrapping)
  {
    // Check every lhs selector against right hand
    for(size_t i = 0, L = length(); i < L; ++i) {
  ...
  void Selector_List_Ref::populate_extends(Selector_List_Obj extendee, Context& ctx, ExtensionSubsetMap& extends)
  {
...
#define IMPLEMENT_AST_OPERATORS(klass) \
  ...
  klass##_Ptr klass##_Ref::clone2() const { \
    klass##_Ptr cpy = copy2(); \
    cpy->cloneChildren(); \
    return cpy; \
  } \
