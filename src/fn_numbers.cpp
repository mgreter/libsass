    ...
    namespace Math {
      ...
      BUILT_IN_FN(round)
      {
        ...
        return SASS_MEMORY_NEW(Number, pstate,
          fuzzyRound(number->value(), logger642.epsilon),
          number->unit());
      }
      ...
      BUILT_IN_FN(max)
      {
        ...
        for (Value* value : arguments[0]->iterator()) {
          Number* number = value->assertNumber(logger642);
          if (max == nullptr || max->lessThan(number, logger642, pstate)) {
            max = number;
          }
        }
      ...
      BUILT_IN_FN(min)
      {
        ...
        for (Value* value : arguments[0]->iterator()) {
          Number* number = value->assertNumber(logger642);
          if (min == nullptr || min->greaterThan(number, logger642, pstate)) {
            min = number;
          }
        }
      ...
      BUILT_IN_FN(random)
      {
        ...
        Number* nr = arguments[0]->assertNumber(logger642, "limit");
        ...
        throw Exception::SassScriptException2(strm.str(), logger642, pstate);
      }
      ...
      {
        Number* number = arguments[0]->assertNumber(logger642, "number");
        number->assertNoUnits(logger642, pstate, "number");
        return SASS_MEMORY_NEW(Number, pstate,
          number->value() * 100, "%");
      }
      ...
      {
        Number* n1 = arguments[0]->assertNumber(logger642, "number1");
        Number* n2 = arguments[1]->assertNumber(logger642, "number2");
