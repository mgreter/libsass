...
namespace Sass {
  ...
  public:
    ...
    Complex_Selector_Ref::Combinator combinator() const { return mCombinator; }

    Complex_Selector_Ptr selector() { return mpSelector; }
    Complex_Selector_Ptr_Const selector() const { return mpSelector; }

    NodeDequePtr collection() { return mpCollection; }
    const NodeDequePtr collection() const { return mpCollection; }

    static Node createCombinator(const Complex_Selector_Ref::Combinator& combinator);
  ...
  private:
    ...
    Node(const TYPE& type, Complex_Selector_Ref::Combinator combinator, Complex_Selector_Ptr pSelector, NodeDequePtr& pCollection);

    TYPE mType;

    // TODO: can we union these to save on memory?
    Complex_Selector_Ref::Combinator mCombinator;
    Complex_Selector_Ptr mpSelector; // this is an AST_Node, so it will be handled by the Memory_Manager
