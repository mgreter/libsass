    ...
    namespace Math {
      ...
      BUILT_IN_FN(fnDiv)
      {
        ...
        if (number1 == nullptr || number2 == nullptr) {
          compiler.addWarning("math.div() will only support number arguments in a future release.\n"
      ...
      void registerFunctions(Compiler& ctx)
        ...
        module.addFunction(key_div, ctx.createBuiltInFunction(key_div, "$base, $exponent", fnDiv));
