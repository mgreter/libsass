...
//    std::unordered_map<Expression_Obj, Expression_Obj>::const_iterator begin() const { return elements_.begin(); }
  ...
  public:
    ...
    : Has_Block(pstate, b), selector2_()
    { statement_type(KEYFRAMERULE); }
    Keyframe_Rule(const Keyframe_Rule* ptr)
    : Has_Block(ptr), selector2_(ptr->selector2_)
    { statement_type(KEYFRAMERULE); }
  ...
  public:
    ...
    { statement_type(IF); }
    If(const If* ptr)
    : Has_Block(ptr), predicate_(ptr->predicate_), alternative_(ptr->alternative_)
    { statement_type(IF); }
    virtual bool has_content()
    {
  ...
  public:
    ...
    Compound_Selector& operator<<(Simple_Selector_Ptr element);
