...
namespace Sass {
  ...
  void SubsetMap2::put(const CompoundSelector_Obj& sel, const SubSetMapPair2& value)
  {
    if (sel->empty()) throw std::runtime_error("internal error: subset map keys may not be empty");
    size_t index = values_.size();
    values_.push_back(value);
  ...
  std::vector<SubSetMapPair2> SubsetMap2::get_kv(const CompoundSelector_Obj& sel)
  {
    SimpleSelectorDict dict(sel->begin(), sel->end()); // XXX Set
    std::vector<size_t> indices;
    for (size_t i = 0, S = sel->length(); i < S; ++i) {
      ...
      const std::vector<std::pair<CompoundSelector_Obj, size_t> >& subsets = hash_[(*sel)[i]];
      for (const std::pair<CompoundSelector_Obj, size_t>& item : subsets) {
        bool include = true;
        for (const Simple_Selector_Obj& it : item.first->elements()) {
          auto found = dict.find(it);
          if (found == dict.end()) {
    ...
    std::vector<SubSetMapPair2> results;
    for (size_t i = 0, S = indices.size(); i < S; ++i) {
      results.push_back(values_[indices[i]]);
    }
    return results;
  ...
  std::vector<SubSetMapPair2> SubsetMap2::get_v(const CompoundSelector_Obj& sel)
  {
    return get_kv(sel);
  }
