...
namespace Sass {
  ...
  CommaSequence_Selector_Ptr Parser::parse_selector_list(bool in_root)
  {
    bool reloop = true;
  ...
  Sequence_Selector_Ptr Parser::parse_complex_selector(bool in_root)
  {

    String_Ptr reference = 0;
    lex < block_comment >();

    Sequence_Selector_Ptr sel = SASS_MEMORY_NEW(ctx.mem, Sequence_Selector, pstate);

    // parse the left hand side
    SimpleSequence_Selector_Ptr lhs = 0;
    ...
    Sequence_Selector_Ref::Combinator combinator;
    if      (lex< exactly<'+'> >()) combinator = Sequence_Selector_Ref::ADJACENT_TO;
    else if (lex< exactly<'~'> >()) combinator = Sequence_Selector_Ref::PRECEDES;
    else if (lex< exactly<'>'> >()) combinator = Sequence_Selector_Ref::PARENT_OF;
    else if (lex< sequence < exactly<'/'>, negate < exactly < '*' > > > >()) {
      // comments are allowed, but not spaces?
      combinator = Sequence_Selector_Ref::REFERENCE;
    ...
    else /* if (lex< zero >()) */   combinator = Sequence_Selector_Ref::ANCESTOR_OF;

    if (!lhs && combinator == Sequence_Selector_Ref::ANCESTOR_OF) return 0;
  ...
  SimpleSequence_Selector_Ptr Parser::parse_compound_selector()
  {
