...
namespace Sass {
  ...
  public:
    ...
    virtual bool is_superselector_of(Compound_Selector* compound);
    virtual bool is_superselector_of(Complex_Selector* complex);
    virtual bool is_superselector_of(Selector_List* list);
...
//      else cerr << "SERIOUSELY " << "\n";
  ...
  public:
    ...
    Complex_Selector* first() {
      ...
      while (s && s->head() && s->head()->size() == 1 && dynamic_cast<Parent_Selector*>((*s->head())[0])) {
        s = s->tail();
      }
      return s;
    };
